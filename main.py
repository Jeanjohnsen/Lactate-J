...
class LactateTestApp:
    ...
    def create_compare_tab(self):
        ...
        ttk.Button(
            button_frame,
            text="Upload Old Test",
            command=self.upload_old_test).grid(
            row=0,
            column=0,
            padx=5,
            pady=5,
        ...
        ttk.Button(
            button_frame,
            text="Compare Tests",
            command=self.compare_tests).grid(
            row=0,
            column=1,
            padx=5,
            pady=5,
    ...
    def create_graph_tab(self):
        ...
        self.left_canvas.bind(
            "<Enter>", lambda event: self._bind_mouse_wheel(
                event, self.left_canvas))
        self.left_canvas.bind(
            "<Leave>", lambda event: self._unbind_mouse_wheel(
                event, self.left_canvas))
        self.right_canvas.bind(
            "<Enter>", lambda event: self._bind_mouse_wheel(
                event, self.right_canvas))
        self.right_canvas.bind(
            "<Leave>", lambda event: self._unbind_mouse_wheel(
                event, self.right_canvas))
        ...
        self.left_canvas.create_window(
            (0, 0), window=self.plot_frame, anchor="nw")
    ...
    def compare_tests(self):
        ...
        if not self.old_data["lactate"]:
            messagebox.showerror(
                "Error", "No old test data available for comparison.")
            return
        ...
        improvements = {
            "FTP": (
                new_ftp -
                old_ftp) if new_ftp and old_ftp else None,
            "LT1": (
                new_lt1 -
                old_lt1) if new_lt1 and old_lt1 else None,
            "LT2": (
                new_lt2 -
                old_lt2) if new_lt2 and old_lt2 else None,
            "FATmax": (
                new_fatmax -
                old_fatmax) if new_fatmax and old_fatmax else None}
